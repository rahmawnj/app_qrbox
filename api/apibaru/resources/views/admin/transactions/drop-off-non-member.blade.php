@props([
    'items' => ['admin', 'Transaksi', 'Transaksi Drop Off Non-Member'],
    'title' => 'Transaksi Drop Off Non-Member',
    'subtitle' => 'Kelola dan pantau seluruh transaksi drop off non-member.',
])
...
@section('content')
    ...
    <div class="row">
        ...
        <div class="col-md-3">
            <div class="widget widget-stats bg-yellow mb-10px">
                ...
                <div class="stats-content">
                    <div class="stats-title">Pendapatan Tunai</div>
                    <div class="stats-number">Rp{{ number_format($totalCashRevenue, 0, ',', '.') }}</div>
                    <div class="stats-desc">Pendapatan dari transaksi tunai.</div>
                </div>
            </div>
        </div>

        {{-- Card 3: Total Pendapatan Non-Tunai --}}
        <div class="col-md-3">
            <div class="widget widget-stats bg-purple mb-10px">
                <div class="stats-icon stats-icon-lg"><i class="fa fa-credit-card fa-fw"></i></div>
                <div class="stats-content">
                    <div class="stats-title">Pendapatan Non-Tunai</div>
                    <div class="stats-number">Rp{{ number_format($totalNonCashRevenue, 0, ',', '.') }}</div>
                    <div class="stats-desc">Pendapatan dari transaksi non-tunai.</div>
                </div>
            </div>
        </div>

        {{-- Card 4: Perangkat Belum Aktif (status: false) --}}
        <div class="col-md-3">
    ...
    <div class="panel panel-inverse">
        ...
        <div class="panel-body">
            <form method="GET" action="{{ route('admin.transactions.drop-off.non-member') }}" class="mb-3">
                ...
                <div class="row g-2 mb-2">
                    ...
                    <div class="col-md-4">
                        <input type="text" name="search" class="form-control form-control-sm"
                            placeholder="Cari (ID Pesanan, Pelanggan, Catatan)"
                ...
                <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
                    <div>
                        ...
                        <a href="{{ route('admin.transactions.drop-off.non-member') }}" class="btn btn-default btn-sm me-2">
                            <i class="fa fa-times"></i> Reset
                        </a>
                    </div>
                    <div class="ms-auto">
                        <a href="{{ route('export.admin-transactions', array_merge(request()->query(), ['type' => 'drop_off', 'status' => 'success', 'payment_method' => 'non_member'])) }}"
                    ...
                    <tbody>
                        @forelse ($transactions as $dropOffTransaction)
                            @php
                                ...
                                if ($dropOffTransaction->payment_type == 'cash') {
                                    $paymentMethodDisplayName = 'Tunai';
                                    $paymentMethodBadgeClass = 'badge-cash';
                                } elseif ($dropOffTransaction->payment_type == 'non_cash') {
                                    $paymentMethodDisplayName = 'Non-Tunai';
                                    $paymentMethodBadgeClass = 'badge-non-cash';
                                }
                            ...
                            <tr>
                                ...
                                <td>
                                    <strong>{{ $dropOffTransaction->customer_name ?? 'N/A' }}</strong><br>
                                    <small class="text-muted">
                                        Catatan: {{ Str::limit($dropOffTransaction->notes ?? 'N/A', 30) }}
                                    </small>
                                </td>
                                <td>Rp {{ number_format($dropOffTransaction->service_price ?? 0, 0, ',', '.') }}</td>
                                <td>
                                ...
                                <td>
                                    ...
                                    {{-- Modal Detail Transaksi Drop Off Non-Member --}}
                                    <div class="modal fade" id="dropOffDetailModal{{ $transaction->id }}" tabindex="-1" role="dialog"
                                         aria-labelledby="dropOffDetailModalLabel{{ $transaction->id }}" aria-hidden="true">
                                                    ...
                                                    <div class="row">
                                                        <div class="col-md-6">
                                                            ...
                                                            <dl class="row">
                                                                ...
                                                                <dd class="col-sm-8">Rp {{ number_format($dropOffTransaction->service_price ?? 0, 0) }}</dd>
                                                        ...
                                                        <div class="col-md-6">
                                                            ...
                                                            <dl class="row">
                                                                ...
                                                                <dd class="col-sm-8">{{ $dropOffTransaction->customer_name ?? 'N/A' }}</dd>

                                                                <dt class="col-sm-4">Telepon Pelanggan:</dt>
                                                                <dd class="col-sm-8">{{ $dropOffTransaction->customer_phone ?? 'N/A' }}</dd>
...
@endforeach
                        ...
                        @empty
                            <tr>
                                <td colspan="9" class="text-center py-4">Tidak ada transaksi drop off non-member ditemukan.</td>
                            </tr>
                        @endforelse
                    </tbody>
                    <tfoot>
