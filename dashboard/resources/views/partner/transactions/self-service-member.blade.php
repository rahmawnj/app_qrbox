@props([
    'items' => ['Partner', 'Transaksi', 'Transaksi Self Service'],
    'title' => 'Transaksi Self Service',
    'subtitle' => 'Kelola dan pantau seluruh transaksi self service (QRIS & Member).',
])
...
@push('styles')
    ...
    <style>
        ...
        .badge-qris {
            /* New badge style for QRIS in self-service */
    ...
    <script>
        $(function() {
            ...
            $('#export-button').on('click', function(e) {
                ...
                var exportUrl =
                    "{{ route('export.partner-transactions', array_merge(request()->query(), ['type' => 'self_service', 'status' => 'success'])) }}";
...
@section('content')
    ...
    <div class="panel panel-inverse">
        ...
        <div class="panel-body">
            ...
            <form method="GET" action="{{ route('partner.transactions.self-service.member') }}" class="mb-3">
                ...
                <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
                    <div>
                        ...
                        <a href="{{ route('partner.transactions.self-service.member') }}" class="btn btn-default btn-sm me-2">
                            <i class="fa fa-times"></i> Reset
                        </a>
                    </div>
                    <div class="ms-auto">
                        <a href="{{ route('export.partner-transactions', array_merge(request()->query(), ['type' => 'self_service', 'status' => 'success'])) }}"
                ...
                <table class="table table-striped table-hover">
                    <thead>
                        <tr>
                            ...
                            <th>Owner & Outlet</th>
                            <th>Jumlah</th>
                            <th>Tipe Pembayaran</th> {{-- Kolom baru untuk tipe pembayaran --}}
                            <th>Status</th>
                            <th>Tanggal</th>
                            <th>Aksi</th>
                        </tr>
                    ...
                    <tbody>
                        @forelse ($transactions as $transaction)
                            @php
                                ...
                                $qrisDetail = null; // Initialize qrisDetail to null

                                if ($transaction->payments->isNotEmpty()) {
                                    $payment = $transaction->payments->first(); // Ambil pembayaran pertama
                                    if ($payment->payment_method === 'member') {
                                        $paymentMethodDisplayName = 'Member';
                                        $paymentMethodBadgeClass = 'badge-self-service'; // Atau badge khusus member
                                    } elseif ($payment->payment_method === 'non_member') {
                                        $paymentMethodDisplayName = 'QRIS';
                                        $paymentMethodBadgeClass = 'badge-qris'; // Atau badge khusus QRIS
                                        // Try to get QRIS detail from the payment
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <small class="text-muted">
                                        ...
                                        @if ($transaction->selfServiceTransaction)
                                            Device: {{ $transaction->selfServiceTransaction->device_code ?? 'N/A' }}
                                        @endif
                                    </small>
                                </td>
                                ...
                                <td>
                                    ...
                                    <button type="button" class="btn btn-info btn-sm" data-bs-toggle="modal"
                                        data-bs-target="#selfServiceDetailModal{{ $transaction->id }}">
                                        <i class="fa fa-info-circle"></i>
                                    </button>
                                    {{-- Modal Detail Transaksi Self Service --}}
                                    <div class="modal fade" id="selfServiceDetailModal{{ $transaction->id }}"
                                        tabindex="-1" role="dialog"
                                        aria-labelledby="selfServiceDetailModalLabel{{ $transaction->id }}"
                                        aria-hidden="true">
                                        <div class="modal-dialog modal-lg" role="document">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h5 class="modal-title"
                                                        id="selfServiceDetailModalLabel{{ $transaction->id }}">Detail
                                                        Transaksi Self Service #{{ $transaction->order_id }}</h5>
                                                ...
                                                <div class="modal-body">
                                                    <div class="row">
                                                        ...
                                                        <div class="col-md-6">
                                                            <h6><i class="fa fa-cogs"></i> Detail Pembayaran & Pelanggan
                                                            </h6>
                                                            <hr class="mt-1 mb-2">
                                                            <dl class="row">
                                                                ...
                                                                {{-- Mengakses detail QRIS melalui payment->qrisTransactionDetail --}}
                                                                @if ($qrisDetail)
                                                                    <dt class="col-sm-4">URL Pembayaran:</dt>
                                                                    <dd class="col-sm-8"><a
                                                                            href="{{ $qrisDetail->payment_url }}"
                                                                            target="_blank">{{ Str::limit($qrisDetail->payment_url, 40) }}</a>
                                                                    </dd>
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="text-center py-4">Tidak ada transaksi self service ditemukan.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
